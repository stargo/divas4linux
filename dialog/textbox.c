...
dialog_textbox (const char *title, const char *file, int height, int width)
{
    ...
    dialog = _dialog_newwin (height, width, y, x, 0);
    ...
    _dialog_draw_box (dialog, 0, 0, height, width, dialog_attr, border_attr);
    ...
    _dialog_title (dialog, width, title);
    _dialog_print_button (dialog, " EXIT ", height - 2, width / 2 - 4, TRUE);
    wnoutrefresh (dialog);
    getyx (dialog, cur_y, cur_x);	/* Save cursor position */

    /* Print first page of text */
    _dialog_attr_clear (text, height - 4, width - 2, dialog_attr);
...
#ifdef HAVE_NCURSES
    if (use_shadow)
	_dialog_draw_shadow (win, y, x, box_height, box_width);
#endif
    _dialog_draw_box (win, y, x, box_height, box_width, dialog_attr,
